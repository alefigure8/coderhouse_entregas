...
export default class CartsResponseDTO {
  constructor(product) {
    let productDestructuring = product[0] || product;
    this.id = productDestructuring._id || productDestructuring.id,
    this.products = productDestructuring.products.map((prod) => {
      return {
        product: {
          id: prod.product._id || prod.product.id,
          title: prod.product.title,
